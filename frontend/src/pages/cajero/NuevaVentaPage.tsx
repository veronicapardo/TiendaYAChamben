...
export function NuevaVentaPage({ usuario, onNavigate, onLogout }: Props) {
  ...
  const [tipoMensajeVenta, setTipoMensajeVenta] = useState<"error" | "exito">("error");
  ...
  const cambioMixto =
  metodoPago === "MIXTO" && totalMixto > total
    ? totalMixto - total
    : 0;
  ...
  try {
    ...
    if (metodoPago === "EFECTIVO" && montoRecibidoNumero < total) {
      mostrarMensaje("error", "El monto recibido no cubre el total de la venta.");
      return;
    }
    ...
    if (metodoPago === "MIXTO" && totalMixto < total) {
      mostrarMensaje("error", "El pago mixto no cubre el total de la venta.");
      return;
    }
    ...
    const ventaRapida: CreateVentaRapidaDto = {
      clienteNombre: cliente.trim() !== "" ? cliente : "Cliente caja",
      observaciones,
...
if (tipoComprobante === "FACTURA") {
  ...
  mostrarMensaje("exito", "Venta registrada correctamente.");
    ...
    if (error instanceof Error) {
      ...
      mostrarMensaje("error", "Error inesperado al registrar la venta.");
    }
  }
}
    ...
    <main className="cajero-dashboard">
      <aside className="cajero-sidebar">
        ...
        <nav className="cajero-menu">
          ...
          <button className="menu-item active" onClick={() => onNavigate("nueva-venta")}>
...
>
  <LogOut size={18} />
      ...
      <section className="cajero-content">
        <header className="dashboard-header">
          <div>
            <h1>
              Nueva venta <ShoppingCart size={28} />
            </h1>
            <p>Registrar una venta rápida en caja</p>
          </div>
        ...
        <section className="venta-layout">
          ...
          <div className="venta-panel carrito-panel">
            ...
            <div className="carrito-lista">
              {carrito.length === 0 && (
                <p className="venta-info">Todavía no agregaste productos.</p>
              )}
  ...
  <button
    ...
    onClick={() => setMetodoPago("QR_TRANSFERENCIA")}
  >
    <CreditCard size={22} />
...
{metodoPago === "QR_TRANSFERENCIA" && (
  <div className="pago-digital-box">
    ...
    <input
      ...
      onChange={(evento) => setReferenciaPago(evento.target.value)}
  ...
  {faltaMixto > 0
    ...
    : cambioMixto > 0
      ...
      : "Pago completo"}
            ...
            <div className="comprobante-box">
              <label>
                <input
                  ...
                  onChange={() => setTipoComprobante("TICKET")}
                />
              ...
              {tipoComprobante === "FACTURA" && (
                <div className="factura-form">
                  ...
                  <input
                    value={nitCi}
                    onChange={(evento) => setNitCi(evento.target.value)}
            ...
            {mensajeVenta && (
  <div className={`mensaje-venta ${tipoMensajeVenta}`}>
    {mensajeVenta}
  </div>
)}
            ...
            <button className="btn-cancelar" onClick={vaciarCarrito}>
              <X size={22} />
              Cancelar
            </button>
          </div>
                ...
                <section className="venta-bottom">
          <div>
            <Users size={22} />
          ...
          <div>
            ...
            <input
              ...
              placeholder="Agregar observaciones de la venta..."
            />
          </div>
        </section>
      </section>
